...
fn test_analysis__blame__should_name_the_author_of_the_likely_responsible_commit() {
    ...
    let blame_json = r#"{"narrative":"Commit by Jane Developer changed auth.rs to return 200.","suggested_fix":null,"likely_commit_hash":"deadbeef1234","likely_commit_message":"refactor: simplify auth responses","likely_commit_author":"Jane Developer <jane@example.com>"}"#;

    let base = std::env::temp_dir()
        .join(format!("ought_blame_author_{}", std::process::id()));
    ...
    assert!(
        ...
        "blame should identify a likely-responsible commit so its author can be named"
    );
