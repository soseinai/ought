...
fn test_ought__llm_agnostic__must_be_agnostic_to_which_llm_provider_generates_the_test_code() {
    ...
    assert_eq!(result_a.clause_id, result_b.clause_id);
    assert_eq!(result_a.language, result_b.language);
    ...
    assert_eq!(parsed[0].clause_id, ClauseId("ought::llm_agnostic::agnostic_clause".to_string()));
}
...
fn test_ought__llm_agnostic__must_support_at_least_anthropic_claude_and_openai_as_providers() {
    ...
    for alias in &["anthropic", "claude"] {
        ...
        assert!(result.is_ok(), "from_config(\"{alias}\", None) must return Ok for Anthropic alias; got: {:?}", result.err());
    }

    for alias in &["openai", "chatgpt"] {
        let result = from_config(alias, None);
        assert!(result.is_ok(), "from_config(\"{alias}\", None) must return Ok for OpenAI alias; got: {:?}", result.err());
    }

    assert!(from_config("anthropic", Some("claude-sonnet-4-6")).is_ok());
    assert!(from_config("openai", Some("gpt-4o")).is_ok());
...
fn test_ought__llm_agnostic__should_support_local_models_via_ollama() {
    use ought_gen::generator::Generator;
    use ought_gen::providers::{from_config, ollama::OllamaGenerator};

    let default_result = from_config("ollama", None);
    assert!(default_result.is_ok(), "from_config(\"ollama\", None) must return Ok with default model; got: {:?}", default_result.err());

    for model in &["llama3", "mistral", "codellama", "phi3", "gemma2"] {
        let result = from_config("ollama", Some(model));
        assert!(result.is_ok(), "from_config(\"ollama\", Some(\"{model}\")) must return Ok; got: {:?}", result.err());
    }
